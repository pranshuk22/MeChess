...
def test_dial_returns_table_rows_at_table_points_and_clamps_outside():
    for elo, (nodes, k, window, temp, scale, book) in dial.DEFAULT_TABLE.items():
        s = dial.settings_for(elo)
        assert (s.nodes, s.multipv, s.window_cp, s.book_plies) == (nodes, k, window, book)
        assert s.temperature == pytest.approx(temp) and s.cp_scale == pytest.approx(scale)
    assert dial.settings_for(400) == dial.settings_for(1200).__class__(400, *[getattr(dial.settings_for(1200), f) for f in
                                                                          ("nodes", "multipv", "window_cp", "temperature", "cp_scale", "book_plies")])
    assert dial.settings_for(3500).nodes == dial.settings_for(2600).nodes
