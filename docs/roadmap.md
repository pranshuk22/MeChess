...
- MeChess controller: book, engine candidates, prior, sampling, Elo dial (placeholder table), UCI front end.
...
2. **Measure our engine's strength** against Stockfish at known settings.
3. **Calibrate the Elo dial** with matches against reference engines and later a Lichess bot account; state the rating scale honestly.
